...
T = {
    "en": {
        ...
        "contrib": "Edit `data/catalog.json` (one object per service, see `data/schema.json`), run `python scripts/build.py`, open a pull request. Every new entry must link to the vendor's official API documentation.",
    ...
    "pl": {
        ...
        "contrib": "Edytuj `data/catalog.json` (jeden obiekt na usługę, zob. `data/schema.json`), uruchom `python scripts/build.py` i otwórz pull request. Każdy nowy wpis musi wskazywać oficjalną dokumentację API dostawcy.",
        "h_sources": "Przypisy",
