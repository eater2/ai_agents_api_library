...
- [ ] **(ręcznie, właściciel)** Włączyć GitHub Pages: Settings → Pages → Branch `main`, folder `/docs`
- [ ] **(ręcznie, właściciel)** Opis, strona domowa i tematy repo (`ai-agents`, `mcp`, `mcp-servers`, `model-context-protocol`, `api-catalog`, `llms-txt`, `tool-calling`, `llm-tools`) — albo dodać repo do uprawnień tokena `GITHUB_TOKEN` (Administration + Pages: write), wtedy zrobi to skrypt
